...
bool GraphCreator::Create() {
  // 有2种格式，一种是openstreet格式，通过OpendriveAdapter来读取，另外一种是apollo自己定义的格式
  ...
  for (const auto& lane : pbmap_.lane()) {
    ...
    // 存储图中节点index和lane_id的关系，因为通过node可以找到lane，而通过lane_id需要遍历节点才能找到节点index
